...
typedef struct {int theta_elec;     	// Output: Motor Electrical angle (Q15)  
                ...
                float (*calc)();    		// Pointer to the calc funtion        
                }  POSSPEED;
...
#if (CPU_FRQ_150MHZ)
  #define POSSPEED_DEFAULTS {0x0,0x0,0x0,0x0,0x0,16777,3,0,0x0,\
        94,0,6000,0,\
        0,0,0,\
        (void (*)(long))POSSPEED_Init,\
        (float (*)(long))POSSPEED_Calc }
#endif
#if (CPU_FRQ_100MHZ)
  #define POSSPEED_DEFAULTS {0x0,0x0,0x0,0x0,0x0,16777,3,0,0x0,\
        63,0,6000,0,\
...
float POSSPEED_Calc(POSSPEED_handle);
